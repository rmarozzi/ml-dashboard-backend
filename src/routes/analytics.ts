...
router.get("/", requireAuth, requirePlan("ouro"), requireFuncionarioPermission("view_analytics"), async (req, res) => {
  ...
  const orders = await prisma.order.findMany({
    where: { tokenId: { in: tokenIds }, dateCreated: { gte: from, lte: to }, status: { not: "cancelled" } },
    include: { items: true, payments: true },
    orderBy: { dateCreated: "asc" },
  });

  // Pré-carrega custos por SKU
  ...
  // Pré-carrega alíquota global (efetiva por data)
  ...
  function calcOrderProfit(order: typeof orders[number]) {
    ...
    const mlFee = order.items.reduce((acc, item) => acc + (item.saleFee ?? 0) * item.quantity, 0);
  ...
  for (const order of orders) {
    ...
    revenue += r.grossRevenue; cost += r.cost; profit += r.profit;
    const key = useMonthGrouping ? order.dateCreated.toISOString().slice(0, 7) : order.dateCreated.toISOString().slice(0, 10);
  ...
  const paymentMethods = paymentGroups
    .map((p) => ({ name: paymentLabel(p.paymentMethodId), value: totalPayments > 0 ? Math.round((p._count.id / totalPayments) * 100) : 0 }))
